...
async def play(client: Client, message_: Message):
    ...
    res = await message_.reply_text("✯𝗗𝗮𝗯𝗩𝗖𝗣𝗹𝗮𝘆✯=🔄 Processing...")
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        ...
        await res.edit_text(f"✯𝗗𝗮𝗯𝗩𝗖𝗣𝗹𝗮𝘆✯=#️⃣ Queued at position {position}.")
    else:
        await res.edit_text("✯𝗗𝗮𝗯𝗩𝗖𝗣𝗹𝗮𝘆✯=▶️ Playing...")
        res.delete
        m = await client.send_photo(
        chat_id=message_.chat.id,
        photo="https://telegra.ph/file/bfd3e51f44415da8875f3.jpg",
        caption=f"Playing Your song Via  [✯𝗗𝗮𝗯𝗩𝗖𝗣𝗹𝗮𝘆✯](https://t.me/TeLeTiPsOfficialOnTopicChat).",
         ) 
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        position = sira.add(message_.chat.id, file_path)
        await res.edit_text(f"✯𝗗𝗔𝗕𝗩𝗖𝗣𝗹𝗮𝘆✯=#️⃣ Queued at position {position}.")
    else:
        await res.edit_text("✯𝗗𝗔𝗕𝗩𝗖𝗣𝗹𝗮𝘆✯=▶️ Playing.....")
        tgcalls.pytgcalls.join_group_call(message_.chat.id, file_path)
    await res.edit("Generating Thumbnail.")
